...
static finline void FileHandle_init(FileHandle* fh)
{
    memset(fh, 0, sizeof(FileHandle));
}


static finline void Command_init(Command* cmd)
{
...
static void pipeline(Lexer* lexer, ArrayPipeline* arpip, Pipeline* pip, memmax depth);



static finline void print_input_and_invalid_token(Lexer* lexer, Token* tok) 
{
...
static finline void expect(Lexer* lexer, ubyte next, memmax bitmask, const char* type)
{
    if(next) next_token(lexer);
    if(BM(lexer->curr.type) & bitmask) return;
...
static void duplicate_or_close(Lexer* lexer, FileHandle* fh)
{
    next_token(lexer);
...
static void redirection(Lexer* lexer, Command* cmd)
{
    ...
    for(;;) {
        ...
        switch(type) {
            case TK_NUMBER: {
                ...
                printf_error("expected string, instead got '%s%s'.", numstr, adjacent);
                jump_out(-1);
            }
            case TK_WORD: {
            ...
            default: {
                printf_error(perrors[ERR_EXPECT], "string", Token_tostr(&lexer->curr));
                print_input_and_invalid_token(lexer, &lexer->curr);
                jump_out(-1);
            }
        }
...
static void command(Lexer* lexer, ArrayPipeline* arpip, Command* cmd, memmax depth)
{
    ArrayCharptr* target;
    ...
    for(;;) {
        ...
        switch(lexer->curr.type) {
            ...
            case TK_NUMBER: {
                if(!number(lexer, cmd)) goto l_switch;
                break;
            }
            default: return;
        }
...
static void pipeline(Lexer* lexer, ArrayPipeline* arpip, Pipeline* pip, memmax depth)
{
    ...
    for(;;) {
        ...
        if(lexer->curr.type != TK_PIPE && lexer->curr.type != TK_PIPE_AND) break;
    }
}


static finline void conditional(Lexer* lexer, Conditional* cond)
{
    Pipeline pip;
    Pipeline_init(&pip);
    for(;;) {
        ...
        if(lexer->curr.type == TK_AND_AND) last->connection = CON_AND;
        else if(lexer->curr.type == TK_PIPE_PIPE) last->connection = CON_OR;
        if(last->connection & CON_NONE) break;
    }
}


int32 parse(const char* str)
{
